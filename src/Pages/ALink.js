    ...
    <Sidebar>
      <div className="alink_editor_container">
        ...
        <div className="alink-flexcontainer">
          ...
          <div>
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mb-10"
            onClick={()=>{
              ...
              if(type=="link"){
                if(action=="create"){
                  ...
                  post(STRINGS.apiurl+"create",formdata).then(result=>{
                    ...
                    const temp = [...menu]
                    temp.splice(parseInt(menu.position),0,{type:"link",id:result.result.insertId,name:data.name})
                    const new_formdata = new FormData()
                    new_formdata.append("data",JSON.stringify({value:temp,type:"menu"}));
                    
                    post(STRINGS.apiurl+"update",new_formdata).then(result=>{
                  ...
                  post(STRINGS.apiurl+"update",formdata).then(result=>{
                    if(parseInt(data.position)!=index1|| menu[parseInt(index1)].name!=data.name){
                ...
                if(action=="create"){
                  ...
                  post(STRINGS.apiurl+"create",formdata).then(result=>{
                    ...
                    const temp = [...menu]
                    temp[parseInt(index1)].items.splice(parseInt(menu.position),0,{type:"link",id:result.result.insertId,name:data.name})
                    const new_formdata = new FormData()
                    new_formdata.append("data",JSON.stringify({value:temp,type:"menu"}));
                  ...
                  post(STRINGS.apiurl+"update",formdata).then(result=>{
                    if(parseInt(data.position)!=index2){
                      const temp = [...menu]
                      const _temp = temp[parseInt(index1)].items[parseInt(data.position)]
                      temp[parseInt(index1)].items.splice(parseInt(index2),1)
                      temp[parseInt(index1)].items.splice([parseInt(data.position),0,_temp])
                      const new_formdata = new FormData()
                      new_formdata.append("data",JSON.stringify({value:temp,type:"menu"}));
            ...
            <label
              ...
              <input
                ...
                onChange={(e) => {
                 setfiles(prev=>({...prev,image_feature:e.target.files[0]}))
            ...
            <label
              ...
              accept="image/*"
                ...
                onChange={(e) => {
                 setfiles(prev=>({...prev, image_banner:e.target.files[0]}))
